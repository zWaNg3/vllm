    ...
    def enqueue_output(self, output: Any):
        ...
        if isinstance(output, AsyncModelRunnerOutput):
            try:
                ...
                # In FT mode the failed forward may still hold pending KV
                # transfer state; carry it to the executor so it is not lost.
                kv_connector_output = output.get_kv_connector_output()
                ft_enabled = (
                    self.worker.vllm_config.parallel_config.enable_fault_tolerance
                )
                if (
                    ft_enabled
